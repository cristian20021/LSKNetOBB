...
model = dict(
    type='OBBTwoStageDetector',
    pretrained='pretrained/lsknet_s_backbone.pth',
    backbone=dict(
        type='LSKNet',
...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.05, nms=dict(type='obb_nms', iou_thr=0.5), max_per_img=2000))
